...
internal static class DbSeeder
{
        ...
        StudentsContext studentsContext)
    {
        ...
        CourseSeedModel[] courseConfigs = config
        ...
        DepartmentSeedModel[] departmentConfigs = config
        ...
        foreach (Course course in coursesContext.Courses)
        {
            string department = courseConfigs.Single(x => x.CourseCode == course.CourseCode).Department;
            course.DepartmentExternalId = departmentsContext.Departments.Single(x => x.Name == department).ExternalId;
        }

        coursesContext.SaveChanges();

        Student[] students = config
        ...
        InstructorSeedModel[] instructorConfigs = config
            .GetSection("DepartmentsContext:Instructors")
            .Get<InstructorSeedModel[]>();

        Instructor[] instructors = instructorConfigs.Select(x => new Instructor
        {
            FirstMidName = x.FirstName,
            LastName = x.LastName,
            HireDate = x.HireDate,
        ...
        foreach (Department department in departmentsContext.Departments)
        {
        ...
        Course[] courses = coursesContext.Courses.AsNoTracking().ToArray();
        ...
        studentsContext.Enrollments.AddRange(config
            ...
            .Select(x => new Enrollment
            {
                ...
                Grade = Enum.TryParse(x.Grade, true, out Grade grade)
                    ? grade
                    : null
            }));
